...
// Read all records (online with fallback offline)
export async function readRecords() {
  if (navigator.onLine) {
    try {
      return await getRecords('pets');
    } catch (error) {
      notifyUser(`Fetching online failed: ${error.message}. Using offline data.`);
...
// Update record (online or offline)
export async function updateRecordById(id, data) {
  if (navigator.onLine) {
    try {
      ...
      notifyUser(`Online update failed: ${error.message}. Saving offline.`);
      data.id = id;
      data.synced = false;
...
// Delete record (online or offline)
export async function deleteRecordById(id) {
  if (navigator.onLine) {
    try {
      ...
      notifyUser(`Online delete failed: ${error.message}. Deleting offline.`);
...
// Sync offline data to Firebase on reconnect
...
// DOM content loaded event — form handler for adding pets
